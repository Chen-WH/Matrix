...
public:
		...
		if (obj1.row != obj2.row || obj1.col != obj2.col) {
			...
			for (int i = 0; i < tmp.row; ++i) {
				for (int j = 0; j < tmp.col; ++j) {
					tmp.index[i*obj1.col + j] = obj1.index[i*obj1.col + j] + obj2.index[i*obj2.col + j];
				}
			}
			return tmp;
		}
	...
	friend Matrix operator+(const Matrix& obj, const float X) {
		...
		for (int i = 0; i < tmp.row; ++i) {
			for (int j = 0; j < tmp.col; ++j) {
				tmp.index[i*obj.col + j] = obj.index[i*obj.col + j] + X;
			}
		}
		return tmp;
	}
	...
	friend Matrix operator*(const Matrix& obj1, const Matrix& obj2) {
		if (obj1.col != obj2.row) {
			...
			for (int i = 0; i < tmp.row; ++i) {
				for (int j = 0; j < tmp.col; ++j) {
					tmp.index[i*obj2.col + j] = 0;
					for (int k = 0; k < obj1.col; ++k) {
						tmp.index[i*obj2.col + j] = tmp.index[i*obj2.col + j] + obj1.index[i*obj1.col + k] * obj2.index[k*obj2.col + j];
					}
				}
			}
			return tmp;
	...
	friend Matrix operator*(const Matrix& obj, const float X) {
		...
		for (int i = 0; i < tmp.row; ++i) {
			for (int j = 0; j < tmp.col; ++j) {
				tmp.index[i*tmp.col + j] = obj.index[i*obj.col + j] * X;
			}
		}
		return tmp;
	}
	...
	Matrix block(int X, int Y, int new_row, int new_col) {
		...
		for (int i = 0; i < new_row; ++i) {
			memcpy(tmp.index + i*new_col, index + (i + X)*col, new_col* sizeof( float ));
		}
		return tmp;
	}
	// 矩阵赋值
	...
	void swaprow(const int X, const int Y) {
		...
		memcpy(index + X*col, tmp, col* sizeof( float ));
	}
	//行变换
	...
	//求解线性方程组
	bool solve(const Matrix B, Matrix X) {
		bool flag = true;
		Matrix A(*this);
		int i, j;
		for (i = 0; i < A.row; ++i) {
			j = i;
			while (j < A.row && A.index[j*A.col + i] == 0) {
				++j;
			}
			/*
			if (j == A.row) {
			...
			*/
			...
			//将对角位置转换为1
			float tmp = 1.0 / A.index[i*A.col + i];
			X.rowmultik(i, tmp);
			A.rowmultik(i, tmp);
			//将该列非对角位置转换为0
